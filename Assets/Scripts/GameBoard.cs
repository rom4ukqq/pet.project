...
public class GameBoard : MonoBehaviour
{
    ...
    {
        _size = size;
        _ground.localScale = new Vector3(size.x, size.y, z: 1f);

        Vector2 offset = new Vector2(x: (size.x - 1) * 0.5f, y: (size.y - 1) * 0.5f);
        ...
        for (int i = 0, y = 0; y < size.y; y++)
        {
            for (int x = 0; x < size.x; x++, i++)
            {
                ...
                tile.transform.localPosition = new Vector3(x: x - offset.x, y: 0f, z: y - offset.y);

                if (x > 0)
                {
                    GameTile.MakeEastWestNeighbors(east: tile, west: _tiles[i - 1]);
                }

                if (y > 0)
                {
                    GameTile.MakeNorthWestNeighbors(north: tile, _tiles[i - size.x]);
                }
